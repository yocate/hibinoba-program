...
import { getDefaultIgnorePatterns, getIgnoreAssetPatterns, globAssetFiles, } from '../processor/compile.js';
import { fetchLinkedPublicationManifest, getJsdomFromUrlOrFile, ResourceLoader, } from '../processor/html.js';
import { assertPubManifestSchema, debug, DetailError, log, logError, logUpdate, pathEquals, useTmpDirectory, } from '../util.js';
...
export function writePublicationManifest(output, options) {
    ...
    debug('writePublicationManifest', output, JSON.stringify(encodedManifest, null, 2));
    try {
        assertPubManifestSchema(encodedManifest);
    }
    catch (error) {
        const thrownError = error;
        throw new DetailError(`Validation of publication manifest failed. Please check the schema: ${output}`, typeof thrownError === 'string'
            ? thrownError
...
export async function supplyWebPublicationManifestForWebbook({ entryHtmlFile, outputDir, ...config }) {
    ...
    const manifest = writePublicationManifest(upath.join(outputDir, MANIFEST_FILENAME), {
        title,
        author,
        language,
...
export async function copyWebPublicationAssets({ exportAliases, outputs, copyAsset, themesDir, manifestPath, input, outputDir, entries, }) {
    ...
    const manifest = decodePublicationManifest(JSON.parse(fs.readFileSync(actualManifestPath, 'utf8')));
    ...
    sortManifestResources(manifest);
    fs.writeFileSync(actualManifestPath, JSON.stringify(encodePublicationManifest(manifest), null, 2));
    return { manifest, actualManifestPath };
}
export async function buildWebPublication({ target, ...config }) {
    let outputDir;
    ...
    if (target.format === 'epub') {
        await exportEpub({
            ...
            entryHtmlFile,
            manifest,
            relManifestPath: actualManifestPath && upath.relative(outputDir, actualManifestPath),
            target: target.path,
